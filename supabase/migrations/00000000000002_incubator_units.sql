-- Migration for Incubator Units

CREATE TYPE incubator_type AS ENUM ('AUTOMATIC', 'MANUAL', 'HYBRID');
...
CREATE TABLE IF NOT EXISTS incubators (
    ...
    controller_type incubator_type NOT NULL DEFAULT 'AUTOMATIC',
...
-- If status is string we can just update it, but if it is an ENUM we might need to add values.
-- Standard text is mostly used in these templates but assuming string.
-- Adding constraint to link egg_batches to incubators.
DO $$
BEGIN
    IF NOT EXISTS (
